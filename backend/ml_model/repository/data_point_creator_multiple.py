...
class MultiFeatureDataPointCreator(DataPointCreator):
    ...
    def data_point_list(self) -> list:
        ...
        for (
            feature1_code,
            feature2_code,
        ), metrics in self.metric_frame.by_group.iterrows():
            f1_label = model_util.get_mapped_label(
                self.mappings, str(self.feature1)[:-2], feature1_code
            )
            data_point = datapoint_creator(
                metrics, f1_label, self.mappings, feature2_code, self.inputs
            )
...
def datapoint_creator(
    ...
    data_point = DataPoint(
        f1_label, f2_label, rounded_metrics[0], rounded_metrics[1], rounded_metrics[2]
    )
    return data_point
