...
def mock_file_reader(mock_data):
    ...
    mock_reader.read_file.return_value = (mock_data, mock_data[['gender', 'age']], mock_data['action_status'])
...
def mock_data_processor(mock_data):
    ...
    mock_processor.get_mappings.return_value = {'gender': {'male': 0, 'female': 1}, 'race': {'asian': 0, 'black': 1}}
    mock_processor.drop_categorical_columns.return_value = mock_data.drop(columns=['gender', 'race'])
    return mock_processor

@pytest.fixture
def mock_safe_train_test_split():
    # Mock the train-test split behavior
    with patch('ml_model.repository.safe_train_grid.safe_train_test_split') as mock_split:
        mock_split.return_value = (pd.DataFrame(), pd.DataFrame(), pd.Series(), pd.Series())
        yield mock_split
...
def mock_fairness_evaluator():
    ...
    mock_evaluator.evaluate_fairness.return_value = MagicMock(by_group=pd.DataFrame({
...
def mock_data_point_creator():
    ...
    with patch('ml_model.repository.data_point_creator_multiple.MultiFeatureDataPointCreator') as mock_creator:
...
@patch('ml_model.repository.data_point_creator_multiple.MultiFeatureDataPointCreator', new_callable=MagicMock)
def test_model(
    ...
    mock_file_reader.read_file.return_value = (mock_data, mock_data[['gender', 'age']], mock_data['action_status'])
    ...
    mock_data_processor.get_mappings.return_value = {'gender': {'male': 0, 'female': 1}, 'race': {'asian': 0, 'black': 1}}
    mock_data_processor.drop_categorical_columns.return_value = mock_data.drop(columns=['gender', 'race'])

    # Arrange: Mock the train-test split and grid search
    mock_safe_train_test_split.return_value = (mock_data[['age', 'race']], mock_data[['age', 'race']], mock_data['action_status'], mock_data['action_status'])
    mock_safe_grid_search.return_value = MagicMock(spec=DecisionTreeClassifier)

    # Arrange: Mock the fairness evaluator
    mock_fairness_evaluator.evaluate_fairness.return_value = MagicMock(by_group=pd.DataFrame({
        'accuracy': [0.9, 0.85, 0.88]
    }))

    # Arrange: Mock the data point creator
    mock_data_point_creator.return_value.data_point_list.return_value = [{'feature': 'gender', 'value': 0}]
