...
from ml_model.repository.safe_train_grid import (safe_train_test_split,
                                                 safe_grid_search)  # Adjust import based on your project structure
...
def test_safe_train_test_split_valid_case():
    ...
    assert len(result) == 4, "Expected the result to return four elements (x_train, x_test, y_train, y_test)."
...
def test_safe_grid_search_insufficient_samples():
    ...
    assert result is None, "Expected None when there aren't enough samples for cross-validation."
